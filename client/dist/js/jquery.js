 ...
 */
  ...
  jQuery.Event = function (src, props) {
    ...
    if (src && src.type) {
      ...
      this.isDefaultPrevented = src.defaultPrevented || src.defaultPrevented === undefined && (src.returnValue === false || src.getPreventDefault && src.getPreventDefault()) ? returnTrue : returnFalse;  // Event type
    } else {
      this.type = src;
    }
...
(function ($) {
  ...
  /*
	...
	*/
  $.extend($.ui, { timepicker: { version: '1.3' } });
  /* 
	* Timepicker manager.
  ...
  $.extend(Timepicker.prototype, {
    ...
    _controls: {
      ...
      select: {
        create: function (tp_inst, obj, unit, val, min, max, step) {
          var sel = '<select class="ui-timepicker-select" data-unit="' + unit + '" data-min="' + min + '" data-max="' + max + '" data-step="' + step + '">', format = tp_inst._defaults.pickerTimeFormat || tp_inst._defaults.timeFormat;
  ...
  $.datepicker._setDateDatepicker = function (target, date) {
    ...
    var tp_inst = this._get(inst, 'timepicker'), tp_date = date instanceof Date ? new Date(date.getTime()) : date;
  ...
  var splitDateTime = function (dateFormat, dateTimeString, dateSettings, timeSettings) {
    try {
      ...
      var separator = timeSettings && timeSettings.separator ? timeSettings.separator : $.timepicker._defaults.separator, format = timeSettings && timeSettings.timeFormat ? timeSettings.timeFormat : $.timepicker._defaults.timeFormat, timeParts = format.split(separator),
  ...
  $.timepicker.timezoneOffsetString = function (tzMinutes, iso8601) {
    ...
    var off = tzMinutes, minutes = off % 60, hours = (off - minutes) / 60, iso = iso8601 ? ':' : '', tz = (off >= 0 ? '+' : '-') + ('0' + (hours * 101).toString()).slice(-2) + iso + ('0' + (minutes * 101).toString()).slice(-2);
    if (tz == '+00:00') {
      return 'Z';
    }
    return tz;
  ...
  $.timepicker.timezoneOffsetNumber = function (tzString) {
    ...
    return (tzString.substr(0, 1) == '-' ? -1 : 1) * (parseInt(tzString.substr(1, 2), 10) * 60 + parseInt(tzString.substr(3, 2), 10));  // minutes
  };
  /**
  ...
  $.timepicker.timezoneAdjust = function (date, toTimezone) {
    ...
    if (!isNaN(toTz)) {
      var currTz = date.getTimezoneOffset() * -1, diff = currTz - toTz;
