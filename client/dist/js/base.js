angular.module('base', []).factory('helpers', function () {
  var helpers = {};
  ...
  helpers.serializeParam = function (obj, prefix) {
    ...
    for (var p in obj) {
      ...
      str.push(typeof v == 'object' ? helpers.serializeParam(v, k) : encodeURIComponent(k) + '=' + encodeURIComponent(v));
    }
    return str.join('&');
  };
  ...
  helpers.addUrlPerfix = function (url) {
    var result = '' + (url || '');
    if (url && url.length > 0 && result.indexOf('http://') !== 0 && result.indexOf('https://') !== 0 && result.indexOf('ftp://') !== 0) {
      result = 'http://' + result;
    }
    return result;
  };
